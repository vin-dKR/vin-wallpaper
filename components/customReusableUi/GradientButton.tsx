...
const GradientButton = ({
    ...
    // Map each position to Tailwind classes
    ...
    return (
        <button
            onClick={onClick}
            className={`${positionClasses[position]} flex flex-col items-center justify-center cursor-pointer 
                  transition-transform duration-300 ease-out w-full ${className}`}
        >
            {/* Blurred background */}
            ...
            <div className="relative text-[8px] md:text-xl z-10 bg-gradient-to-b from-white to-black rounded-xl p-[2px] transition-all duration-300 ease-out">
                <div className="bg-gradient-to-br from-white from-5% to-blueBg to-60% px-6 py-2 rounded-xl">
                    <span className="font-bold text-shadow-xl">{text}</span>
                </div>
            </div>
        </button>
    );
